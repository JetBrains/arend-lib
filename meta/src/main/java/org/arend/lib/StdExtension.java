...
public class StdExtension implements ArendExtension {
  ...
  private MetaRef makeRef(ModulePath modulePath, String name, String description) {
    return factory.metaRef(factory.moduleRef(modulePath), name, description);
  }

  private MetaRef makeRef(ModulePath modulePath, String name, String description, Precedence precedence) {
    return factory.metaRef(factory.moduleRef(modulePath), name, precedence, description);
  }

  @Override
  public void declareDefinitions(@NotNull DefinitionContributor contributor) {
    ModulePath meta = new ModulePath("Meta");
    contributor.declare(makeRef(meta, "later", "`later meta args` defers the invocation of `meta args`"), laterMeta);
    contributor.declare(makeRef(meta, "fails",
      """
        `fails meta args` succeeds if and only if `meta args` fails

        `fails {T} meta args` succeeds if and only if `meta args : T` fails
        """), new FailsMeta(this));
    contributor.declare(makeRef(meta, "using", "`using (e_1, ... e_n) e` adds `e_1`, ... `e_n` to the context before checking `e`"), new UsingMeta(true));
    contributor.declare(makeRef(meta, "usingOnly", "`usingOnly (e_1, ... e_n) e` replaces the context with `e_1`, ... `e_n` before checking `e`"), new UsingMeta(false));
    contributor.declare(makeRef(meta, "hiding", "`hiding (x_1, ... x_n) e` hides local variables `x_1`, ... `x_n` from the context before checking `e`"), new HidingMeta());
    contributor.declare(makeRef(meta, "run", "`run { e_1, ... e_n }` is equivalent to `e_1 $ e_2 $ ... $ e_n`"), new RunMeta(this));
    contributor.declare(makeRef(meta, "at", "`((f_1, ... f_n) at x) r` replaces variable `x` with `f_1 (... (f_n x) ...)` and runs `r` in the modified context", new Precedence(Precedence.Associativity.NON_ASSOC, (byte) 1, true)), new AtMeta(this));
    contributor.declare(makeRef(meta, "in", "`f in x` is equivalent to `\\let r => f x \\in r`. Also, `(f_1, ... f_n) in x` is equivalent to `f_1 in ... f_n in x`. This meta is usually used with `f` being a meta such as `rewrite`, `simplify`, `simp_coe`, or `unfold`.", new Precedence(Precedence.Associativity.RIGHT_ASSOC, (byte) 1, true)), new InMeta(this));
    casesMeta = new CasesMeta(this);
    contributor.declare(makeRef(meta, "cases",
      """
        ...
        """), casesMeta);
    contributor.declare(makeRef(meta, "mcases",
      """
        ...
        * `mcases {_} {arg addPath, arg (), arg addPath}` looks for case expressions and adds a path argument after the first and the third matched expression.
        """), new MatchingCasesMeta(this));
    contributor.declare(makeRef(meta, "unfold",
      """
        ...
        """), new DeferredMetaDefinition(new UnfoldMeta(this), true, false));
    contributor.declare(makeRef(meta, "unfold_let", "Unfolds \\let expressions"), new DeferredMetaDefinition(new UnfoldLetMeta(), true, false));
    contributor.declare(makeRef(meta, "unfolds", "Unfolds recursively top-level functions and fields"), new DeferredMetaDefinition(new UnfoldsMeta(), true, false));
    contributor.declare(makeRef(meta, "<|>",
    ...
    contributor.declare(makeRef(meta, "try", "The same as {<|>}"), new OrElseMeta(this));
    contributor.declare(makeRef(meta, "mkcon",
      "Inserts data type arguments in constructor invocation.\n" +
      "If constructor `con` has 3 data type arguments, then `mkcon con args` is equivalent to `con {_} {_} {_} args`"), new MakeConstructorMeta(this));
    contributor.declare(makeRef(meta, "assumption",
      """
        ...
        * `assumption` {n} a1 ... ak applies n-th variable from the context to arguments a1, ... ak.
        """), new AssumptionMeta(this));
    contributor.declare(makeRef(meta, "defaultImpl", "`defaultImpl C F E` returns the default implementation of field `F` in class `C` applied to expression `E`. The third argument can be omitted, in which case either `\\this` or `_` will be used instead,"), new DefaultImplMeta(this));

    ModulePath paths = ModulePath.fromString("Paths.Meta");
    contributor.declare(makeRef(paths, "rewrite",
      """
        ...
        """), new RewriteMeta(this, false, true, false));
    contributor.declare(makeRef(paths, "rewriteI", "`rewriteI p` is equivalent to `rewrite (inv p)`"), new RewriteMeta(this, false, false, false));
    contributor.declare(makeRef(paths, "rewriteEq",
      """
        ...
        """), new RewriteMeta(this, false, true, true));
    contributor.declare(makeRef(paths, "simplify", "Simplifies the expected type or the type of the argument if the expected type is unknown."), new DeferredMetaDefinition(new SimplifyMeta(this), true));
    contributor.declare(makeRef(paths, "simp_coe",
      """
        ...
        * If the goal is `transport (\\lam x => f x = g x) p q = s`, then the subgoal is `q *> pmap g p = pmap f p *> s`. If `f` does not depend on `x`, then the right hand side of the subgoal is simply `s`.
        ...
        """), simpCoeMeta, new ClassExtResolver(this));
    contributor.declare(makeRef(paths, "ext",
      """
        ...
        * If the goal is `x = {P} y`, where `P : \\Prop`, then the argument for {ext} should be omitted
        """), new DeferredMetaDefinition(extMeta, false, ExtMeta.defermentChecker), new ClassExtResolver(this));
    contributor.declare(makeRef(paths, "exts", "Similar to {ext}, but also applies either {simp_coe} or {ext} when a field of a \\Sigma-type or a record has an appropriate type."), new DeferredMetaDefinition(extsMeta, false, ExtMeta.defermentChecker), new ClassExtResolver(this));

    MetaDefinition apply = new ApplyMeta(this);
    ModulePath function = ModulePath.fromString("Function.Meta");
    contributor.declare(makeRef(function, "$", "`f $ a` returns `f a`", new Precedence(Precedence.Associativity.RIGHT_ASSOC, (byte) 0, true)), apply);
    contributor.declare(makeRef(function, "#'", "`f #' a` returns `f a`", new Precedence(Precedence.Associativity.LEFT_ASSOC, (byte) 0, true)), apply);
    contributor.declare(makeRef(function, "repeat",
      """
    ...
    contributor.declare(makeRef(algebra, "equation",
      """
        ...
        """), new DeferredMetaDefinition(equationMeta, true));
    contributor.declare(makeRef(algebra, "linarith", "Solve systems of linear equations"), new DeferredMetaDefinition(linearSolverMeta, true));
    contributor.declare(makeRef(algebra, "cong", "Proves an equality by congruence closure of equalities in the context. E.g. derives f a = g b from f = g and a = b"), new DeferredMetaDefinition(new CongruenceMeta(this)));

    ModulePath logic = ModulePath.fromString("Logic.Meta");
    contributor.declare(makeRef(logic, "contradiction",
      """
    ...
    givenMetaRef = makeRef(logic, "Given",
      """
        ...
        * If `l : Array A`, then `Given (x y : l) (P x y)` is equivalent to `\\Sigma (j j' : Fin l.len) (P (l j) (l j'))`
        """);
    contributor.declare(givenMetaRef, new ExistsMeta(this, ExistsMeta.Kind.SIGMA));
    ...
    contributor.declare(existsMetaRef, new ExistsMeta(this, ExistsMeta.Kind.TRUNCATED));
    forallMetaRef = factory.metaRef(factory.moduleRef(logic), "Forall", Precedence.DEFAULT, "∀", Precedence.DEFAULT,
      """
        ...
        * If `P : A -> \\Type`, then `Forall {x y : P} (Q x) (Q y)` is equivalent to `\\Pi {x y : A} -> P x -> P y -> Q x -> Q y`
        """);
    contributor.declare(forallMetaRef, new ExistsMeta(this, ExistsMeta.Kind.PI));
    constructorMetaRef = makeRef(logic, "constructor", "Returns either a tuple, a \\new expression, or a single constructor of a data type depending on the expected type");
    contributor.declare(constructorMetaRef, new ConstructorMeta(this, false));

    ModulePath debug = ModulePath.fromString("Debug.Meta");
    contributor.declare(makeRef(debug, "time", "Returns current time in milliseconds"), new TimeMeta(this));
    contributor.declare(makeRef(debug, "println", "Prints the argument to the console"), new PrintMeta(this));
    contributor.declare(makeRef(debug, "random",
      """
        `random` returns a random number.
        `random n` returns a random number between 0 and `n`.
        `random (l,u)` returns a random number between `l` and `u`.
        """), new RandomMeta(this));
    MetaRef nfMeta = makeRef(debug, "nf", "Normalizes the argument");
    contributor.declare(nfMeta, new NormalizationMeta(NormalizationMode.ENF));
    contributor.declare(factory.metaRef(nfMeta, "whnf", ""), new NormalizationMeta(NormalizationMode.WHNF));
    contributor.declare(factory.metaRef(nfMeta, "rnf", ""), new NormalizationMeta(NormalizationMode.RNF));

    ModulePath category = ModulePath.fromString("Category.Meta");
    contributor.declare(makeRef(category, "sip", "Proves univalence for categories. The type of objects must extend `BaseSet` and the Hom-set must extend `SetHom` with properties only."), sipMeta);
  }
