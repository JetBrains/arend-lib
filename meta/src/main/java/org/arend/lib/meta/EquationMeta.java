...
public class EquationMeta extends BaseMetaDefinition {
  ...
  public @Nullable ConcreteExpression getConcreteRepresentation(@NotNull List<? extends ConcreteArgument> arguments) {
    ...
    for (int i = 0; i < arguments.size(); i++) {
      if (arguments.get(i).isExplicit() && i + 1 < arguments.size() && arguments.get(i + 1).isExplicit() || !arguments.get(i).isExplicit() && isUsingOrHiding(getMeta(arguments.get(i).getExpression()))) {
    ...
    if (!hasMissingProofs) {
      ...
      for (int i = arguments.size() - 1; i >= 0; i--) {
        if (arguments.get(i).isExplicit()) {
          continue;
        }
        if (result == null) {
          result = arguments.get(i).getExpression();
        } else {
          result = ext.factory.app(ext.factory.ref(ext.concat.getRef()), true, Arrays.asList(arguments.get(i).getExpression(), result));
        }
      }
      return result;
    }
  ...
  private TypedExpression finalizeState(State state, ConcreteExpression expression) {
    ConcreteFactory factory = state.factory;
  ...
  private CompiledTerm compileTerm(State state, CoreExpression expression) {
